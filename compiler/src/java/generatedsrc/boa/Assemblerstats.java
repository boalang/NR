...
public class Assemblerstats extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Assemblerstats: " + out);

		job.setJarByClass(AssemblerstatsBoaMapper.class);

		job.setMapperClass(AssemblerstatsBoaMapper.class);
		job.setCombinerClass(AssemblerstatsBoaCombiner.class);
		job.setReducerClass(AssemblerstatsBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Assemblerstats(), args);
		//System.exit();
	}
	...
	static class AssemblerstatsBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.GFeature.Genome _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				if (___asYear <= 2010l)
				{
					context.write(new boa.io.EmitKey("[" + (___asYear) + "]" + "[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]" + "[" + (___adata.getTotalLength()) + "]" + "[" + (___adata.getTotalGapLength()) + "]" + "[" + (___adata.getScaffoldCount()) + "]" + "[" + (___adata.getScaffoldN50()) + "]" + "[" + (___adata.getContigCount()) + "]" + "[" + (___adata.getContigN50()) + "]", "AsmStats", 0), new boa.io.EmitValue(1l));
				}
	...
	static class AssemblerstatsBoaCombiner extends boa.runtime.BoaCombiner {
		public AssemblerstatsBoaCombiner() {
			super();
	...
	static class AssemblerstatsBoaReducer extends boa.runtime.BoaReducer {
		public AssemblerstatsBoaReducer() {
			super();

			this.aggregators.put("0::AsmStats", new boa.aggregators.CollectionAggregator());
			this.aggregators.put("0::AssemblerCounts", new boa.aggregators.IntSumAggregator());
		}
	}
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new AssemblerstatsBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new AssemblerstatsBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new AssemblerstatsBoaReducer();
	}
}