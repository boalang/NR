...
public class Maxp extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Maxp: " + out);

		job.setJarByClass(MaxpBoaMapper.class);

		job.setMapperClass(MaxpBoaMapper.class);
		job.setCombinerClass(MaxpBoaCombiner.class);
		job.setReducerClass(MaxpBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Maxp(), args);
		//System.exit();
	}
	...
	static class MaxpBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.Nr2.Sequence _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				for (long ___i = 0; ___i < ___s.getClusterList().size(); ___i++)
				{
					...
					{
						{
							context.write(new boa.io.EmitKey("MaxProtein", 0), new boa.io.EmitValue(___s.getSeqid() + " _ " + ___s.getClusterList().get((int)(___i)).getCid(), ___s.getClusterList().get((int)(___i)).getLength()));
						}
					}
				}
	...
	static class MaxpBoaCombiner extends boa.runtime.BoaCombiner {
		public MaxpBoaCombiner() {
			super();

			this.aggregators.put("0::MaxProtein", new boa.aggregators.MaximumAggregator(10l));
		}
	}

	static class MaxpBoaReducer extends boa.runtime.BoaReducer {
		public MaxpBoaReducer() {
			super();

			this.aggregators.put("0::MaxProtein", new boa.aggregators.MaximumAggregator(10l));
			this.aggregators.put("0::ClusterInfo", new boa.aggregators.CollectionAggregator());
		}
	}

	@Override
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new MaxpBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new MaxpBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new MaxpBoaReducer();
	}
}