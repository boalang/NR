...
public class Nr1 extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Nr1: " + out);

		job.setJarByClass(Nr1BoaMapper.class);

		job.setMapperClass(Nr1BoaMapper.class);
		job.setCombinerClass(Nr1BoaCombiner.class);
		job.setReducerClass(Nr1BoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Nr1(), args);
		//System.exit();
	}
	...
	static class Nr1BoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.Nr2.Sequence _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				for (long ___i = 0; ___i < ___s.getAnnotationList().size(); ___i++)
				{
					...
					{
						{
							if (boa.functions.BoaStringIntrinsics.match("hypothetical protein", ___s.getAnnotationList().get((int)(___i)).getDefline()))
							{
								context.write(new boa.io.EmitKey("[" + (___s.getSeqid()) + "]" + "[" + (___s.getAnnotationList().get((int)(___i)).getDefline()) + "]" + "[" + (___s.getAnnotationList().get((int)(___i)).getTaxId()) + "]" + "[" + (___s.getAnnotationList().get((int)(___i)).getTaxName()) + "]", "counts", 0), new boa.io.EmitValue(1l));
							}
						}
					}
				}
	...
	static class Nr1BoaCombiner extends boa.runtime.BoaCombiner {
		public Nr1BoaCombiner() {
			super();
	...
	static class Nr1BoaReducer extends boa.runtime.BoaReducer {
		public Nr1BoaReducer() {
			super();
