...
public class TmpTaxa extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("TmpTaxa: " + out);

		job.setJarByClass(TmpTaxaBoaMapper.class);

		job.setMapperClass(TmpTaxaBoaMapper.class);
		job.setCombinerClass(TmpTaxaBoaCombiner.class);
		job.setReducerClass(TmpTaxaBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new TmpTaxa(), args);
		//System.exit();
	}
	...
	static class TmpTaxaBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.Nr2.Sequence _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				for (long ___i = 0; ___i < ___s.getAnnotationList().size(); ___i++)
				{
					...
					{
						{
							if (boa.functions.BoaStringIntrinsics.match(___s.getAnnotationList().get((int)(___i)).getTaxName(), "Streptococcus"))
							{
	...
	static class TmpTaxaBoaCombiner extends boa.runtime.BoaCombiner {
		public TmpTaxaBoaCombiner() {
			super();
	...
	static class TmpTaxaBoaReducer extends boa.runtime.BoaReducer {
		public TmpTaxaBoaReducer() {
			super();
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new TmpTaxaBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new TmpTaxaBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new TmpTaxaBoaReducer();
	}
}