...
public class Cdhit extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Cdhit: " + out);

		job.setJarByClass(CdhitBoaMapper.class);

		job.setMapperClass(CdhitBoaMapper.class);
		job.setCombinerClass(CdhitBoaCombiner.class);
		job.setReducerClass(CdhitBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Cdhit(), args);
		//System.exit();
	}
	...
	static class CdhitBoaMapper extends boa.runtime.BoaMapper {
					...
					{
						{
							...
							{
								context.write(new boa.io.EmitKey("[" + (___s.getClusterList().get((int)(___i)).getCid()) + "]" + "[" + (___s.getSeqid()) + "]" + "[" + (___s.getClusterList().get((int)(___i)).getSeqStart()) + "]" + "[" + (___s.getClusterList().get((int)(___i)).getSeqStop()) + "]", "counts", 0), new boa.io.EmitValue(1l));
							}
						}
					}
				}
	...
	static class CdhitBoaCombiner extends boa.runtime.BoaCombiner {
		public CdhitBoaCombiner() {
			super();
	...
	static class CdhitBoaReducer extends boa.runtime.BoaReducer {
		public CdhitBoaReducer() {
			super();
