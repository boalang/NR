...
public class StatsAll extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("StatsAll: " + out);

		job.setJarByClass(StatsAllBoaMapper.class);

		job.setMapperClass(StatsAllBoaMapper.class);
		job.setCombinerClass(StatsAllBoaCombiner.class);
		job.setReducerClass(StatsAllBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new StatsAll(), args);
		//System.exit();
	}
	...
	static class StatsAllBoaMapper extends boa.runtime.BoaMapper {
					...
					{
						{
								...
								{
									{
										...
										{
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "geneCounts", 0), new boa.io.EmitValue(1l));
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "geneMean", 0), new boa.io.EmitValue(___flength));
										}
										if (boa.functions.BoaStringIntrinsics.match("exon", ___fdata.getFeatureList().get((int)(___j)).getFtype()))
										{
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "exonCounts", 0), new boa.io.EmitValue(1l));
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "exonMean", 0), new boa.io.EmitValue(___flength));
										}
										if (boa.functions.BoaStringIntrinsics.match("mRNA", ___fdata.getFeatureList().get((int)(___j)).getFtype()))
										{
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "mRNACounts", 0), new boa.io.EmitValue(1l));
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "mRNAMean", 0), new boa.io.EmitValue(___flength));
										}
										if (boa.functions.BoaStringIntrinsics.match("CDS", ___fdata.getFeatureList().get((int)(___j)).getFtype()))
										{
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "CDSCounts", 0), new boa.io.EmitValue(1l));
											context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "CDSMean", 0), new boa.io.EmitValue(___flength));
										}
									}
								}
							}
							...
							for (long ___k = 0; ___k < ___adata.getAssemblerList().size(); ___k++)
							{
								...
								{
									{
										context.write(new boa.io.EmitKey("[" + (___g.getTaxid()) + "]" + "[" + (___adata.getAssemblerList().get((int)(___k)).getName()) + "]", "AssemblerCounts", 0), new boa.io.EmitValue(1l));
									}
								}
							}
	...
	static class StatsAllBoaCombiner extends boa.runtime.BoaCombiner {
		public StatsAllBoaCombiner() {
			super();
	...
	static class StatsAllBoaReducer extends boa.runtime.BoaReducer {
		public StatsAllBoaReducer() {
			super();
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new StatsAllBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new StatsAllBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new StatsAllBoaReducer();
	}
}