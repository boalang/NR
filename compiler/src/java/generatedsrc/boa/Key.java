...
public class Key extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Key: " + out);

		job.setJarByClass(KeyBoaMapper.class);

		job.setMapperClass(KeyBoaMapper.class);
		job.setReducerClass(KeyBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Key(), args);
		//System.exit();
	}
	...
	static class KeyBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.Nr2.Sequence _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				{
					boolean _stop_j = false;
					if (___s.getAnnotationList().get((int)(___j)).getKeyID().equals("AAC45972.1"))
					{
						_stop_j = true;
						{
							for (long ___i = 0; ___i < ___s.getClusterList().size(); ___i++)
							{
								...
								{
									{
										context.write(new boa.io.EmitKey("[" + (___s.getSeqid()) + "]" + "[" + (___s.getClusterList().get((int)(___i)).getCid()) + "]" + "[" + (___s.getClusterList().get((int)(___i)).getSimilarity()) + "]" + "[" + (___s.getClusterList().get((int)(___i)).getSeqStart()) + "]" + "[" + (___s.getClusterList().get((int)(___i)).getSeqStop()) + "]", "counts", 0), new boa.io.EmitValue(1l));
									}
								}
							}
	...
	static class KeyBoaCombiner extends boa.runtime.BoaCombiner {
		public KeyBoaCombiner() {
			super();

		}
	}

	static class KeyBoaReducer extends boa.runtime.BoaReducer {
		public KeyBoaReducer() {
			super();
