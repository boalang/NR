...
public class TopAssembler extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("TopAssembler: " + out);

		job.setJarByClass(TopAssemblerBoaMapper.class);

		job.setMapperClass(TopAssemblerBoaMapper.class);
		job.setCombinerClass(TopAssemblerBoaCombiner.class);
		job.setReducerClass(TopAssemblerBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new TopAssembler(), args);
		//System.exit();
	}
	...
	static class TopAssemblerBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			boa.types.GFeature.Genome ___g;
			boa.types.GFeature.AssemblerRoot ___assemblerData;
			...
			public void map(final boa.types.GFeature.Genome _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				___assemblerData = boa.functions.BoaGenomeIntrinsics.getAssembler(___g.getRefseq());

				for (long ___i = 0; ___i < ___assemblerData.getAssemblerList().size(); ___i++)
				{
					if ((___assemblerData.getAssemblerList().get((int)(___i)) != null))
					{
						{
							context.write(new boa.io.EmitKey("counts", 0), new boa.io.EmitValue(___assemblerData.getAssemblerList().get((int)(___i)).getName(), 1l));
						}
					}
				}
	...
	static class TopAssemblerBoaCombiner extends boa.runtime.BoaCombiner {
		public TopAssemblerBoaCombiner() {
			super();

			this.aggregators.put("0::counts", new boa.aggregators.TopAggregator(3l));
		}
	}

	static class TopAssemblerBoaReducer extends boa.runtime.BoaReducer {
		public TopAssemblerBoaReducer() {
			super();
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new TopAssemblerBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new TopAssemblerBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new TopAssemblerBoaReducer();
	}
}