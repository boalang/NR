...
public class TaxRep extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("TaxRep: " + out);

		job.setJarByClass(TaxRepBoaMapper.class);

		job.setMapperClass(TaxRepBoaMapper.class);
		job.setCombinerClass(TaxRepBoaCombiner.class);
		job.setReducerClass(TaxRepBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new TaxRep(), args);
		//System.exit();
	}
	...
	static class TaxRepBoaMapper extends boa.runtime.BoaMapper {
					...
					{
						{
							if (___s.getClusterList().get((int)(___i)).getRepresentative())
							{
								for (long ___j = 0; ___j < ___s.getAnnotationList().size(); ___j++)
								{
									...
									{
										{
											if (!___s.getAnnotationList().get((int)(___j)).getTaxName().equals(""))
											{
												context.write(new boa.io.EmitKey("[" + (___s.getClusterList().get((int)(___i)).getSimilarity()) + "]", "counts", 0), new boa.io.EmitValue(1l));
											}
										}
									}
								}
	...
	static class TaxRepBoaCombiner extends boa.runtime.BoaCombiner {
		public TaxRepBoaCombiner() {
			super();
	...
	static class TaxRepBoaReducer extends boa.runtime.BoaReducer {
		public TaxRepBoaReducer() {
			super();
