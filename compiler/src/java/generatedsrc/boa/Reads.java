...
public class Reads extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Reads: " + out);

		job.setJarByClass(ReadsBoaMapper.class);

		job.setMapperClass(ReadsBoaMapper.class);
		job.setCombinerClass(ReadsBoaCombiner.class);
		job.setReducerClass(ReadsBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Reads(), args);
		//System.exit();
	}
	...
	static class ReadsBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.GFeature.Genome _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				for (long ___i = 0; ___i < ___g.getSequenceList().size(); ___i++)
				{
					...
					{
						{
							context.write(new boa.io.EmitKey("[" + (___g.getRefseq()) + "]" + "[" + (___g.getTaxid()) + "]", "counts", 0), new boa.io.EmitValue(1l));
						}
					}
				}
	...
	static class ReadsBoaCombiner extends boa.runtime.BoaCombiner {
		public ReadsBoaCombiner() {
			super();
	...
	static class ReadsBoaReducer extends boa.runtime.BoaReducer {
		public ReadsBoaReducer() {
			super();
