...
public class TestFile extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("TestFile: " + out);

		job.setJarByClass(TestFileBoaMapper.class);

		job.setMapperClass(TestFileBoaMapper.class);
		job.setReducerClass(TestFileBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new TestFile(), args);
		//System.exit();
	}
	...
	static class TestFileBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			boa.types.Nr2.Sequence ___s;
			String ___strContent;
			String[] ___ids;
			long ___j;
			...
			public void map(final boa.types.Nr2.Sequence _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				___strContent = boa.functions.BoaNrIntrinsics.readFile("/media/sf_BoaG/01_jupter_Boa/ids");

				___ids = boa.functions.BoaStringIntrinsics.splitall(___strContent, ",");

				for (___j = 0l; ___j < ((long)___ids.length); ___j++)
				{
					if (___s.getSeqid().equals(___ids[(int)(___j)]))
					{
						context.write(new boa.io.EmitKey("[" + (___ids[(int)(___j)]) + "]", "counts", 0), new boa.io.EmitValue(1l));
					}
				}
	...
	static class TestFileBoaCombiner extends boa.runtime.BoaCombiner {
		public TestFileBoaCombiner() {
			super();

		}
	}

	static class TestFileBoaReducer extends boa.runtime.BoaReducer {
		public TestFileBoaReducer() {
			super();
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new TestFileBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new TestFileBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new TestFileBoaReducer();
	}
}