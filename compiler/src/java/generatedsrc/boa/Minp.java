...
public class Minp extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Minp: " + out);

		job.setJarByClass(MinpBoaMapper.class);

		job.setMapperClass(MinpBoaMapper.class);
		job.setCombinerClass(MinpBoaCombiner.class);
		job.setReducerClass(MinpBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Minp(), args);
		//System.exit();
	}
	...
	static class MinpBoaMapper extends boa.runtime.BoaMapper {
					...
					{
						{
							...
							{
								context.write(new boa.io.EmitKey("MinProtein95", 0), new boa.io.EmitValue(___s.getSeqid() + " _ " + boa.functions.BoaCasts.longToString(___s.getClusterList().get((int)(___i)).getSimilarity()), ___s.getClusterList().get((int)(___i)).getLength()));
							}
							if (___s.getClusterList().get((int)(___i)).getSimilarity() == 90l)
							{
								context.write(new boa.io.EmitKey("MinProtein90", 0), new boa.io.EmitValue(___s.getSeqid() + " _ " + boa.functions.BoaCasts.longToString(___s.getClusterList().get((int)(___i)).getSimilarity()), ___s.getClusterList().get((int)(___i)).getLength()));
							}
						}
					}
				}
	...
	static class MinpBoaCombiner extends boa.runtime.BoaCombiner {
		public MinpBoaCombiner() {
			super();

			this.aggregators.put("0::MinProtein90", new boa.aggregators.MinimumAggregator(10l));
			this.aggregators.put("0::MinProtein95", new boa.aggregators.MinimumAggregator(10l));
		}
	}

	static class MinpBoaReducer extends boa.runtime.BoaReducer {
		public MinpBoaReducer() {
			super();
