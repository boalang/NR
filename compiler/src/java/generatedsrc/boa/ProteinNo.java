...
public class ProteinNo extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("ProteinNo: " + out);

		job.setJarByClass(ProteinNoBoaMapper.class);

		job.setMapperClass(ProteinNoBoaMapper.class);
		job.setCombinerClass(ProteinNoBoaCombiner.class);
		job.setReducerClass(ProteinNoBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new ProteinNo(), args);
		//System.exit();
	}
	...
	static class ProteinNoBoaMapper extends boa.runtime.BoaMapper {
					...
					{
						{
							...
							{
								context.write(new boa.io.EmitKey("[" + (___s.getClusterList().get((int)(___i)).getLength()) + "]", "counts", 0), new boa.io.EmitValue(1l));
							}
						}
					}
				}
	...
	static class ProteinNoBoaCombiner extends boa.runtime.BoaCombiner {
		public ProteinNoBoaCombiner() {
			super();
	...
	static class ProteinNoBoaReducer extends boa.runtime.BoaReducer {
		public ProteinNoBoaReducer() {
			super();
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new ProteinNoBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new ProteinNoBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new ProteinNoBoaReducer();
	}
}