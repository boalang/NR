...
public class Clusters extends boa.runtime.BoaRunner {
	/** {@inheritDoc} */
	@Override
	public org.apache.hadoop.mapreduce.Job job(final org.apache.hadoop.fs.Path[] ins, final org.apache.hadoop.fs.Path out, final boolean robust) throws java.io.IOException {
		...
		job.setJobName("Clusters: " + out);

		job.setJarByClass(ClustersBoaMapper.class);

		job.setMapperClass(ClustersBoaMapper.class);
		job.setCombinerClass(ClustersBoaCombiner.class);
		job.setReducerClass(ClustersBoaReducer.class);
	...
	org.apache.hadoop.util.ToolRunner.run(new Clusters(), args);
		//System.exit();
	}
	...
	static class ClustersBoaMapper extends boa.runtime.BoaMapper {
		private static class Job0 implements BoaJob {
			...
			public void map(final boa.types.Nr2.Sequence _input, final org.apache.hadoop.mapreduce.Mapper<org.apache.hadoop.io.Text, org.apache.hadoop.io.BytesWritable, boa.io.EmitKey, boa.io.EmitValue>.Context context) throws Exception {
				...
				for (long ___i = 0; ___i < ___s.getClusterList().size(); ___i++)
				{
					...
					{
						{
							context.write(new boa.io.EmitKey("[" + (___s.getClusterList().get((int)(___i)).getSimilarity()) + "]", "protein_counts", 0), new boa.io.EmitValue(1l));
							if (___s.getClusterList().get((int)(___i)).getRepresentative())
							{
								context.write(new boa.io.EmitKey("[" + (___s.getClusterList().get((int)(___i)).getSimilarity()) + "]", "cluster_counts", 0), new boa.io.EmitValue(1l));
							}
						}
					}
				}
	...
	static class ClustersBoaCombiner extends boa.runtime.BoaCombiner {
		public ClustersBoaCombiner() {
			super();

			this.aggregators.put("0::cluster_counts", new boa.aggregators.IntSumAggregator());
			this.aggregators.put("0::protein_counts", new boa.aggregators.IntSumAggregator());
		}
	}

	static class ClustersBoaReducer extends boa.runtime.BoaReducer {
		public ClustersBoaReducer() {
			super();
	...
	public org.apache.hadoop.mapreduce.Mapper getMapper() {
		return new ClustersBoaMapper();
	}

	@Override
	public boa.runtime.BoaCombiner getCombiner() {
		return new ClustersBoaCombiner();
	}

	@Override
	public boa.runtime.BoaReducer getReducer() {
		return new ClustersBoaReducer();
	}
}